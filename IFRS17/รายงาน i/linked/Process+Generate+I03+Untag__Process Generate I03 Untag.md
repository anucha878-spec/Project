# Process Generate I03 Untag

- url: http://wiki.thaisamut.co.th/display/RDSADW/Process+Generate+I03+Untag
- http status: 200
...
1. ดึงข้อมูลกรมธรรม์หลักจาก [tx_mps_landing_i03](http://wiki.thaisamut.co.th/display/RDSADW/tx_mps_landing_i03) ด้วยเงื่อนไข ดังนี้
2. period = $รอบประมวลผลปัจจุบัน
3. current_basic_policy_status มีสถานะ Active (อ้างอิง: [A5. การกำหนดสถานะกรมธรรม์ Active และ Non Active](http://wiki.thaisamut.co.th/pages/viewpage.action?pageId=917471597))

ตรวจสอบ issue_date ดังนี้
...
1. **กลุ่มกรมธรรม์เคสใหม่, เคสใหม่ที่มีการ Freelook**ดึงข้อมูลกรมธรรม์หลักจาก[tx_mps_landing_i03](http://wiki.thaisamut.co.th/display/RDSADW/tx_mps_landing_i03)ด้วยเงื่อนไข ดังนี้period = $รอบประมวลผลปัจจุบัน
2. issue_date effective_date = วันที่ issue กรมธรรม์ตั้งแต่** 1 ****2** เดือนก่อนหน้าของ $รอบประมวลผลปัจจุบัน จนถึงวันที่สิ้นเดือนก่อนหน้าของ $รอบประมวลผลปัจจุบัน และ effective_date หากพบตามเงื่อนไข (a.) ให้ตรวจสอบ ต้องเป็นกรมธรรม์ที่ยังไม่เคยมีข้อมูลใน Master โดยค้นหาที่ตาราง [tx_mps_master_i03](http://wiki.thaisamut.co.th/display/RDSADW/tx_mps_master_i03) ด้วยเงื่อนไข policy_number = เลขที่กรมธรรม์**กลุ่มกรมธรรม์ Reinstate**
1. ดึงข้อมูลกรมธรรม์หลักจาก [tx_mps_landing_i03](http://wiki.thaisamut.co.th/display/RDSADW/tx_mps_landing_i03) ด้วยเงื่อนไข ดังนี้period = $รอบประมวลผลปัจจุบัน
...
3. ตรวจสอบวันที่ issue date และวันเริ่มสัญญา ต้องไม่เกินวันที่สิ้นเดือนของรอบประมวลผล กรณีมี issue date: issue_date effective_date หากพบตามเงื่อนไข (a.) ให้ตรวจสอบ ต้องเป็นกรมธรรม์ที่ยังไม่เคยมีข้อมูลใน Master โดยค้นหาที่ตาราง [tx_mps_master_i03](http://wiki.thaisamut.co.th/display/RDSADW/tx_mps_master_i03) ด้วยเงื่อนไข policy_number = เลขที่กรมธรรม์
...
5. movement = 'A' (A = Add) ไม่ใช่ค่า 'R' หรือ 'U'
...
นำกรมธรรม์จากข้อ a. ไปตรวจสอบ ต้องเป็นกรมธรรม์ที่เคยมีข้อมูลใน Master โดยค้นหาที่ตาราง [tx_mps_master_i03](http://wiki.thaisamut.co.th/display/RDSADW/tx_mps_master_i03) ด้วยเงื่อนไข policy_number = เลขที่กรมธรรม์ แต่ไม่เคยมีข้อมูล RCG Tagging จาก [tx_mps_master_i03](http://wiki.thaisamut.co.th/display/RDSADW/tx_mps_master_i03) ด้วยเงื่อนไข ri_portfolio_id เป็นค่าว่าง

>
**2. บันทึกข้อมูลกรมธรรม์หลักที่ตาราง untag**

นำ List กธ.ที่ผ่านเงื่อนไขจากข้อ 1. มาเตรียมข้อมูล เพื่อบันทึกลงตาราง [tx_mps_untag_i03](http://wiki.thaisamut.co.th/display/RDSADW/tx_mps_untag_i03) โดยแยกเงื่อนไขการเตรียมข้อมูลเป็น 2 กรณี ดังนี้
...
1. นำข้อมูลกรมธรรม์หลักจาก [tx_mps_landing_i03](http://wiki.thaisamut.co.th/display/RDSADW/tx_mps_landing_i03) ด้วยเงื่อนไข period = $รอบประมวลผลปัจจุบัน และ policy_number = เลขที่กรมธรรม์จากข้อ 1.1.1
2. บันทึกข้อมูลที่ตาราง untag ตามเงื่อนไข mapping field (อ้างอิง Field จาก Column "Input Untag" จาก [EDW-MPS-PS003-02 เงื่อนไขการบันทึกข้อมูลกรมธรรม์ PA](http://wiki.thaisamut.co.th/pages/viewpage.action?pageId=933232906))

>
**2.2 กรณีขอ RCG Tagging เพิ่มเติม**

1. ดึงข้อมูลสำหรับบันทึกข้อมูล untag ดังนี้ข้อมูลกรมธรรม์หลักจาก [tx_mps_landing_i03](http://wiki.thaisamut.co.th/display/RDSADW/tx_mps_landing_i03)ด้วยเงื่อนไข period = $รอบประมวลผลปัจจุบัน และ policy_number = เลขที่กรมธรรม์จากข้อ 1.1.2
2. ข้อมูล ICG Tagging และข้อมูลสำหรับคำนวณเกี่ยวกับสถานะกรมธรรม์ ณ สิ้นเดือนก่อนของกรมธรรม์ จาก [tx_mps_master_i03](http://wiki.thaisamut.co.th/display/RDSADW/tx_mps_master_i03)ด้วยเงื่อนไข policy_number
บันทึกข้อมูลที่ตาราง untag ตามเงื่อนไข mapping field (อ้างอิง Field จาก Column "Input Untag" จาก [EDW-MPS-PS003-02 เงื่อนไขการบันทึกข้อมูลกรมธรรม์ PA](http://wiki.thaisamut.co.th/pages/viewpage.action?pageId=933232906))
...
3. report_type มีค่าเป็น 'New', 'Renew', 'New(Fac)', 'Renew(Fac)' โดย convert ให้เป็น UPPERCASE ทั้งหมดทั้ง Key และ Value
4. code_plan = แบบประกัน
5. movement = 'A' ไม่ใช่ค่า 'R' หรือ 'U'
6. ri_std_hd_id = [tx_ri_std_hd](http://wiki.thaisamut.co.th/display/RDSADW/Process+Generate+I04+Untag).ri_std_hd_id และต้องเป็น header ที่มีสถานะนำเข้าสำเร็จ ([tx_ri_std_hd](http://wiki.thaisamut.co.th/display/RDSADW/Process+Generate+I04+Untag).ri_process_hd_id = [tx_ri_process_header](http://wiki.thaisamut.co.th/display/RDSADW/Process+Generate+I04+Untag).ri_process_hd_id และ [tx_ri_process_header](http://wiki.thaisamut.co.th/display/RDSADW/Process+Generate+I04+Untag).ri_process_status_id = 3) tx_ri_std_hd_oic.ri_std_hd_id และต้องเป็น header ที่มีสถานะนำเข้าสำเร็จ (tx_ri_std_hd_oic.ri_process_hd_id = tx_ri_process_header_oic.ri_process_hd_id และ tx_ri_process_header_oic.ri_process_status_id = 3)
...
2. ดำเนินการต่อในข้อ** 4. กรองข้อมูลกธ.ที่ duplicate**** **และ** 5. Generate ข้อมูลกรมธรรม์ออกมาเป็น I03 File ในรูปแบบ .csv**
...
Update ข้อมูล RI ที่ตาราง [tx_mps_untag_i03](http://wiki.thaisamut.co.th/display/RDSADW/tx_mps_untag_i03) ด้วยเงื่อนไข period = $รอบประมวลผลปัจจุบัน, policy_number = เลขที่กรมธรรม์, plan_code = แบบประกัน โดย update ค่า ดังนี้

| Update [tx_mps_untag_i03](http://wiki.thaisamut.co.th/display/RDSADW/tx_mps_untag_i03) | Source: [tx_ri_support_bk_dt](http://wiki.thaisamut.co.th/display/RDSADW/tx_ri_support_bk_dt)tx_ri_support_bk_oic | Remarks |
| --- | --- | --- |
...
5. nar_hd_id = nar_hd_id in (select nar_hd_id from tx_mps_nar_hd where ms_import_status_id = '7' and transaction_type = 'New business') กรณีพบข้อมูลจาก [tx_mps_nar_ord_landing](http://wiki.thaisamut.co.th/display/RDSADW/tx_mps_nar_ord_landing) ตามเงื่อนไขข้างต้น ให้ Update ข้อมูล NAR ที่ตาราง **tx_mps_untag_i03 **ดังนี้ | Source: [tx_mps_nar_ord_landing](http://wiki.thaisamut.co.th/display/RDSADW/tx_mps_nar_ord_landing) | Data untag condition | Update **tx_mps_untag_i03** | | --- | --- | --- | | ri_previous_nar_1 | if ri_prev_net_amount_at_risk_1
...
3. nar_hd_id = nar_hd_id in (select nar_hd_id from tx_mps_nar_hd where ms_import_status_id = '7' and transaction_type = 'New business') กรณีพบข้อมูลจาก [tx_mps_nar_ord_landing](http://wiki.thaisamut.co.th/display/RDSADW/tx_mps_nar_ord_landing) ตามเงื่อนไขข้างต้น ให้ Update ข้อมูล NAR ที่ตาราง **tx_mps_untag_i03 **ดังนี้ | Source: [tx_mps_nar_ord_landing](http://wiki.thaisamut.co.th/display/RDSADW/tx_mps_nar_ord_landing) | Data untag condition | Update **tx_mps_untag_i03** | | --- | --- | --- | | treaty_code_1 | if treaty_code_1 is null | treaty_code_1 | | ri_previous_nar_1 | if ri_prev_net_amount_at_risk_1
...
กรณีพบข้อมูลตามเงื่อนไขข้างต้น ให้ Update ข้อมูล NAR ที่ตาราง **tx_mps_untag_i03 **ดังนี้
...
ระบบจะตรวจสอบสถานะกรมธรรม์ปัจจุบันเพิ่มเติมว่าเป็น D-P ([tx_mps_untag_i03](http://wiki.thaisamut.co.th/display/RDSADW/tx_mps_untag_i03).current_policy_status = 'D-P') หรือไม่

1. กรณีที่เข้าเงื่อนไข ระบบจะต้อง Update ค่า Actual SA = 0 ([tx_mps_untag_i03](http://wiki.thaisamut.co.th/display/RDSADW/tx_mps_untag_i03).actual_sum_insured = 0)
...
1. ดึงข้อมูลจาก [tx_mps_untag_i03](http://wiki.thaisamut.co.th/display/RDSADW/tx_mps_untag_i03) ด้วยเงื่อนไข period = $รอบประมวลผลปัจจุบัน
2. ตรวจสอบหากพบจำนวนของเลขที่กรมธรรม์ใด มากกว่า 1 รายการ บันทึกข้อมูลที่ [tx_mps_untag_i03_dup](http://wiki.thaisamut.co.th/display/RDSADW/tx_mps_untag_i03_dup)
3. ลบข้อมูลจาก [tx_mps_untag_i03](http://wiki.thaisamut.co.th/display/RDSADW/tx_mps_untag_i03)

>
**5. Generate ข้อมูลกรมธรรม์ออกมาเป็น I03 File ในรูปแบบ .csv**
...
2. ชื่อ Column อ้างอิงจาก ตาราง output ที่ Column "I03 Report Column Name"
3. Value ให้นำค่าจากตาราง Untag มาบันทึก (อ้างอิง Field จาก Column "Output Untag" จาก [EDW-MPS-PS003-02 เงื่อนไขการบันทึกข้อมูลกรมธรรม์ PA](http://wiki.thaisamut.co.th/pages/viewpage.action?pageId=933232906))